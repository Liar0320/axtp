...
| `0x03` | `READY` | Client→Server | 可选 | 预留 | 三步协商预留：Client 通知 Server 协议层就绪，当前版本不实现 |
...
`READY`（`0x03`）为三步协商预留字段，当前版本不实现。收到 `READY` 时必须忽略，不得返回错误。
...
| `0x0203` | `RESERVED_CONTROL_BODY_ENCODING_UNSUPPORTED`（历史保留，新实现不得产生） |
...
OPEN 协商协议运行时能力，不协商业务能力。业务能力在 APP_READY 后通过 `capability.getAll` 查询。
...
03 01 01        // headerProfile = STANDARD
...
ACK 示例（确认 Frame）：
```text
opcode=0x05 controlId=0x0002 statusCode=0x0000
Body: 20 01 01 / 11 02 7B 00 / 12 01 03  // targetType=FRAME, messageId=123, frameIndex=3
```

NACK 示例（CRC 错误）：
```text
opcode=0x06 controlId=0x0003 statusCode=0x0106  // FRAME_CRC_ERROR
Body: 20 01 01 / 11 02 7B 00 / 12 01 03         // targetType=FRAME, messageId=123, frameIndex=3
```
...
```text
READY(0x03，预留，收到时忽略) / RESUME(0x08) / RESUME_ACK(0x09) / SESSION_RESET(0x0C) / WINDOW_UPDATE(0x0D) / PING(0x0E) / PONG(0x0F) / GOAWAY(0x10) / VENDOR(0x7F)
```
