...
所有字段 Little-Endian。`payloadLength = 16 + dataLength`，dataLength 由 L1 payloadLength 推导，不在 L2 Header 中重复。
...
| `seqId` | uint32 | 4B | stream 内数据包序号，per-stream，从 0 开始，uint32 自然回绕 |
...
Stream Context 是通过 RPC 创建的流描述对象，保存 streamId 的全部业务语义：
...
STREAM 数据包只携带 `streamId/seqId/cursor/data`，通用 StreamParser 只解析 16B Header，然后按 streamId 查表投递数据。
...
| `file.upload` | `byteOffset` | `reliable` | 可选 |
| `file.download` | `byteOffset` | `reliable` | 可选 |
...
- 对象级校验（业务层）：OTA/File 通过 `firmware.begin.sha256` + `firmware.verify` 完成端到端校验
- Chunk 级校验（可选）：通过 CONTROL ACK/NACK 携带 crc，或在 Profile 数据尾部追加 crc32

MVP 推荐：HID/BLE OTA 使用 Compact Frame CRC8 + stop_and_wait ACK + final sha256；WebSocket/TCP OTA 使用 Standard Frame CRC16 + sliding_window ACK + final sha256。

---

## 10. 断点续传

接收端保存：`streamId`, `profile`, `lastSeqId`, `lastCursor`, `objectId`, `hash`, `resumeToken`。
...
BLE 默认 ATT MTU 23B，去除 ATT 头后有效载荷仅 20B，无法容纳 Compact Frame Header(4B) + STREAM L2 Header(16B) + CRC8(1B) = 21B。

在 BLE 上使用 STREAM 必须在 CONTROL OPEN 阶段协商 MTU，协商后有效 Payload 空间必须 ≥ 22B。推荐协商至 185B 或 247B（BLE 5.0 DLE）。

| BLE ATT MTU | 可用数据 | 有效载荷比 |
|---|---|---|
| 23B（默认） | 2B | 9% |
| 64B | 43B | 67% |
| 185B | 164B | 89% |
...
parse AXTP Frame Header
  ...
  → read streamId(uint32) / seqId(uint32) / cursor(uint64)
  → data = payload[16:]
...
| 错误 | 处理 |
|---|---|
| `payloadLength < 16` | 丢弃，CONTROL NACK |
...
STREAM Header 固定 16B，不得修改字段顺序或长度。新增业务流类型只扩展 Stream Profile Registry，不改 Header。

如未来极低带宽场景需要压缩 L2 Header，应作为 AXTP v2 或 negotiated extension 处理，不进入 MVP。
