...
- Standard / Compact 两种 Header Profile 的字段定义
...
// 正确
payloadType = RPC,    methodId = brightness.set
...
实际单帧长度由 Header Profile、Transport MTU、PayloadLength 字段宽度、设备内存和协商结果共同决定。
...
Transport Connected
    ↓
Client → CONTROL OPEN   （协商 protocolVersion / headerProfile / maxFrameSize /
                          supportedPayloadTypes / supportedRpcEncodings /
                          ackMode / windowSize / heartbeatInterval）
    ↓
...
OPEN 只协商协议运行时能力；业务能力通过 `RPC capability.*` 查询。
...
PAYLOAD_LENGTH_MISMATCH / CRC_ERROR
FRAGMENT_OUT_OF_RANGE / FRAGMENT_TIMEOUT / MESSAGE_REASSEMBLY_FAILED
```
...
Transport Connected → CONTROL OPEN → CONTROL ACCEPT
    → RPC capability.getAll → RPC device.getInfo
