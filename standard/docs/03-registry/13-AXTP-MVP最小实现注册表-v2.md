...
| `0x02` | `RESPONSE` | 必须 |
| `0x03` | `EVENT` | 必须 |
| `0x04` | `BATCH_REQUEST` | 延后 |
| `0x05` | `BATCH_RESPONSE` | 延后 |
...
P0 必须实现：`device.getInfo / device.getVersion / device.getStatus / capability.getAll / brightness.get / brightness.set / stream.open / stream.close / firmware.getInfo / firmware.begin / firmware.end / firmware.verify / firmware.abort`

P1 建议实现：`capability.getDomain / brightness.getRange / stream.getStatus / firmware.apply / firmware.resume / firmware.getProgress / event.subscribe / event.unsubscribe`

P2 延后：`video.* / audio.* / file.* / log.* / diagnostic.* / input.* / network.* / storage.*`
...
| `0x0601` | `brightness.control` | `bool` | 必须 |
| `0x0602` | `brightness.range` | `range` | 建议 |
...
```text
BrightnessGetRequest / BrightnessGetResponse
BrightnessSetRequest / BrightnessSetResponse
BrightnessGetRangeRequest / BrightnessGetRangeResponse
...
如果产品必须保持 wire-level CmdValue 不变，可以采用旧 CmdValue 作为 methodId；如果希望进入 AXTP 新编号体系，则必须在 legacyMapping 中保存 oldCmdValue。
