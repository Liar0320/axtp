...
| Control | OPEN / ACCEPT / HEARTBEAT / ACK / NACK / CLOSE | RESUME / WINDOW_UPDATE 高级策略 |
...
| `0x0103` | `device.getStatus` | `device` | 必须 | 获取设备运行状态 |
...
| `0x0503` | `display.getBrightnessRange` | `display` | 建议 | 获取亮度范围 |
| `0x0901` | `stream.open` | `stream` | 必须 | 打开 Stream |
| `0x0902` | `stream.close` | `stream` | 必须 | 关闭 Stream |
| `0x0903` | `stream.getStatus` | `stream` | 建议 | 查询 Stream 状态 |
...
| `0x0B05` | `firmware.apply` | `firmware` | 建议 | 应用固件 |
| `0x0B06` | `firmware.abort` | `firmware` | 必须 | 中止升级 |
| `0x0B07` | `firmware.resume` | `firmware` | 建议 | 断点续传 |
| `0x0B08` | `firmware.getProgress` | `firmware` | 建议 | 查询升级进度 |

### 7.2 MVP 方法分级

P0 必须实现：`device.getInfo / device.getVersion / device.getStatus / capability.getAll / display.getBrightness / display.setBrightness / stream.open / stream.close / firmware.getInfo / firmware.begin / firmware.end / firmware.verify / firmware.abort`

P1 建议实现：`capability.getDomain / display.getBrightnessRange / stream.getStatus / firmware.apply / firmware.resume / firmware.getProgress`
...
| `0x8507` | `display.brightnessChanged` | `display` | 必须 | 亮度变化 |
...
| `0x0003` | `NOT_SUPPORTED` | common | false | 当前设备或当前模式不支持 |
| `0x0005` | `BUSY` | common | true | 设备忙 |
| `0x0006` | `TIMEOUT` | common | true | 超时 |
| `0x000A` | `INVALID_ARGUMENT` | common | false | 参数非法 |
| `0x000B` | `OUT_OF_RANGE` | common | false | 参数越界 |
| `0x000C` | `NOT_FOUND` | common | false | 资源不存在 |
| `0x0106` | `FRAME_CRC_ERROR` | frame | true | Frame CRC 错误 |
| `0x0104` | `FRAME_LENGTH_INVALID` | frame | false | Frame 长度错误 |
| `0x0105` | `FRAME_PAYLOAD_TYPE_INVALID` | frame | false | 不支持的 PayloadType |
...
| `0x0303` | `RPC_PAYLOAD_INVALID` | rpc | false | RPC 请求非法 |
| `0x030B` | `RPC_PARAM_INVALID` | rpc | false | 参数或 Schema 不匹配 |
| `0x0401` | `STREAM_NOT_FOUND` | stream | false | Stream Context 不存在 |
| `0x0408` | `STREAM_SEQ_INVALID` | stream | true | Stream 序号非法或不连续 |
| `0x0403` | `STREAM_CRC_ERROR` | stream | true | Chunk CRC 错误 |
| `0x0402` | `STREAM_TIMEOUT` | stream | true | Stream 超时 |
| `0x0601` | `FW_IMAGE_INVALID` | firmware | false | 固件镜像非法 |
| `0x060B` | `FW_VERIFY_FAILED` | firmware | false | 固件校验失败 |
| `0x060C` | `FW_APPLY_FAILED` | firmware | false | 固件应用失败 |
...
| capabilityId | capabilityName | Type | MVP |
|---:|---|---|---|
| `0x0001` | `protocol.version` | `uint8` | 必须 |
| `0x0002` | `protocol.payloadTypes` | `bitmap32` | 必须 |
| `0x0003` | `protocol.headerProfiles` | `bitmap32` | 必须 |
| `0x0004` | `protocol.rpcEncodings` | `bitmap32` | 必须 |
| `0x0005` | `protocol.streamProfiles` | `list<uint16>` | 必须 |
| `0x0006` | `protocol.maxFrameSize` | `uint16` | 必须 |
| `0x0007` | `protocol.maxPayloadSize` | `uint16` | 必须 |
| `0x0008` | `protocol.mtu` | `uint16` | 必须 |
| `0x0009` | `protocol.ackModes` | `bitmap32` | 必须 |
| `0x000A` | `protocol.windowSize` | `uint16` | 建议 |
| `0x000B` | `protocol.frameCrcProfiles` | `bitmap32` | 必须 |
| `0x000C` | `protocol.messageIdWidths` | `bitmap32` | 必须 |

### 10.2 业务能力

| capabilityId | capabilityName | Type | MVP |
|---:|---|---|---|
| `0x0101` | `device.info` | `bool` | 必须 |
| `0x0102` | `device.version` | `bool` | 必须 |
| `0x0301` | `capability.query` | `bool` | 必须 |
| `0x0601` | `display.brightness` | `bool` | 必须 |
| `0x0602` | `display.brightnessMin` | `uint16` | 建议 |
| `0x0603` | `display.brightnessMax` | `uint16` | 建议 |
| `0x0604` | `display.brightnessStep` | `uint16` | 建议 |
| `0x0901` | `stream.control` | `bool` | 必须 |
| `0x0902` | `stream.ota` | `bool` | 必须 |
| `0x0B01` | `firmware.info` | `bool` | 必须 |
| `0x0B02` | `firmware.update` | `bool` | 必须 |
| `0x0B03` | `firmware.resume` | `bool` | 建议 |
...
|---:|---|---|---|---|---|
| `0x0101` | `firmware.ota` | firmware | 必须 | `byteOffset` | reliable |
| `0x0201` | `file.upload` | file | 建议 | `byteOffset` | reliable |
| `0x0202` | `file.download` | file | 建议 | `byteOffset` | reliable |
| `0x0401` | `log.realtime` | log | 建议 | `timestampUs` | best_effort |
| `0x8001` | `legacy.tunnel` | legacy | 延后 | `byteOffset` | reliable |
