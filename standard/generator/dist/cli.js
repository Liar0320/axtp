...
function resolveInputPath(input) {
    ...
    if (existsSync(direct) || !process.env.INIT_CWD)
        return direct;
    return path.resolve(process.env.INIT_CWD, input);
}
function resolveOutputPath(input) {
    if (path.isAbsolute(input) || !process.env.INIT_CWD)
        return path.resolve(input);
    return path.resolve(process.env.INIT_CWD, input);
}
async function loadAndValidate(specPath) {
    const spec = await loadSpec(resolveInputPath(specPath));
    const messages = validateSpec(spec);
    return { spec, messages };
}
async function loadAndValidateProtocol(specPath) {
...
function defaultProtocolOut(specPath) {
