...
function mapErrorCode(item, file) {
    return {
        id: normalizeId(item.id, `${file}:${item.name}`),
        name: String(item.name),
        domain: String(item.domain),
...
function mapCapability(item, file) {
    return {
        id: normalizeId(item.id, `${file}:${item.name}`),
        name: String(item.name),
        domain: String(item.domain),
...
function mapField(item, file, schemaName) {
    return {
...
export async function loadSpec(specRoot) {
    ...
    const [payloadTypeDoc, controlOpcodeDoc, rpcEncodingDoc, rpcBodyEncodingDoc, rpcOpDoc, streamProfileDoc, methodDoc, eventDoc, errorDoc, capabilityDoc, legacyDoc, mvpDoc, commonSchemaDoc, controlSchemaDoc, deviceSchemaDoc, capabilitySchemaDoc, displaySchemaDoc, firmwareSchemaDoc, streamSchemaDoc, eventSchemaDoc, sessionSchemaDoc] = await Promise.all([
