...
export function normalizeId(value, context) {
...
export function cppConstName(name) {
    return name
...
export function sortById(items) {
    return [...items].sort((a, b) => a.id - b.id || String(a.name).localeCompare(String(b.name)));
}
export async function writeTextFile(filePath, content) {
    await mkdir(path.dirname(filePath), { recursive: true });
    await writeFile(filePath, content.endsWith("\n") ? content : `${content}\n`, "utf8");
}
export function toJsonStable(value) {
