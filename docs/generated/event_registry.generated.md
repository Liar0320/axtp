# Event Registry

| eventId | name | domain | status | schema |
|---:|---|---|---|---|
| `0x8507` | `display.brightnessChanged` | display | mvp | DisplayBrightnessChangedEvent |
| `0x8901` | `stream.opened` | stream | draft | StreamOpenedEvent |
| `0x8903` | `stream.error` | stream | draft | StreamErrorEvent |
| `0x8B02` | `firmware.updateProgress` | firmware | mvp | FirmwareUpdateProgressEvent |
| `0x8B03` | `firmware.updateCompleted` | firmware | mvp | FirmwareUpdateCompletedEvent |
| `0x8B04` | `firmware.updateFailed` | firmware | mvp | FirmwareUpdateFailedEvent |
