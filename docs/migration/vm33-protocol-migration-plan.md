...
> Source evidence: `docs/legacy-protocols/VM33协议文档.md`
> Classification reference: `docs/legacy-classification/by-source/vm33_http_json.md`
...
| 协议事实源 | 新业务事实进入 `docs/protocol/**` 草案，评审后进入 `registry/domains/**`，再由 Generator 输出。 |
| 旧迁移产物 | `AXTP_Legacy_Migration_Matrix.xlsx`、`docs/migration/generated/**` 和旧 VM33 分类材料保留为证据和参考，不作为 VM33 Pro 新协议事实源。 |
...
- 不修改 `docs/legacy-protocols/VM33协议文档.md`。
- 不修改已有老 VM33 迁移矩阵、旧分类产物或 `docs/migration/generated/**`。
- 不把 VM33 旧 `Class.Method` 或 `Config.Name` 批量注册成 AXTP method。
- 不把旧 `Param` 结构直接当成 AXTP schema。
- 不要求本轮修改 `registry/**/*.yaml`、`protocol/axtp.protocol.yaml` 或 generated 产物。
...
```text
需求 / 旧 VM33 证据
  -> docs/protocol/<domain>/<domain.feature>.md 草案
  -> 评审确认 method/event/schema/error/capability
  -> registry/domains/<domain>/domain.yaml
  -> Generator 输出 docs/generated/protocol.md/json 和 runtime generated headers
  -> VM33 Pro handler 注入
  -> App 适配新 method
```
...
`docs/legacy-classification/by-source/vm33_http_json.md` 已把旧 VM33 条目归入候选 AXTP domain.feature。示例：
