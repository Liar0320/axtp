...
> Source evidence: `docs/legacy-protocols/NearHub-Launcher数字标牌设备管理通用管理命令.md`, `docs/legacy-protocols/NearHub-Launcher设备管理命令.md`
> Classification reference: `docs/legacy-classification/by-source/signage_sdk.md`, `docs/legacy-classification/signage.md`
...
- 不手写 `docs/generated/**`、`docs/migration/generated/**` 或 runtime generated headers。
- 不把 `docs/migration/generated/registry-patches.generated.yaml` 直接当作稳定协议事实源。
...
| 状态 | 能力 | 说明 |
|---|---|---|
| 已在当前 registry/generated 中出现 | `device.info`, `firmware.ota` | 可复用现有正式事实，但 signage 的字段映射仍需检查是否完全覆盖旧 SDK 返回。 |
...
| `RemoteUpgrade` | Server -> Device | `firmware.beginOta` / current generated `firmware.begin` | URL 升级走 `source.type=url`，不新增专用 `RemoteUpgrade`。 |
| `UpgradeProgress` | Server -> Device | `firmware.getOtaState` / OTA progress event | 若 App 仍要主动查询进度，采纳 query method；否则改用事件。 |
...
| P0 common | `device.info`, `network.ip`, `storage.sdCard`, `audio.volume`, `audio.input`, `firmware.ota`, `firmware.updatePolicy`, `auth.session`, `log.export` |
...
- 已存在正式方法的能力优先复用，例如 `device.getInfo` 和当前 `firmware.ota` 兼容方法。
...
1. 基础信息、时间、网络、存储、音频、OTA、更新策略、绑定、日志。
...
| `signage-ota-url` | URL 升级通过 `firmware.beginOta` 或当前兼容名启动并可查询/上报进度。 |
