...
| 设置设备显示名 | writable part of `device.info` |
...
如果它回答“这台机器现在怎么运行，能不能重启、关机、休眠或下电”，放到 `system`。

Examples:

| Requirement | Domain direction |
|---|---|
| CPU、内存、温度、健康状态 | `system.state` candidate |
| 电池、供电、电源状态 | `system.state` or `system.power` candidate |
| 定时关机、定时重启 | `system.lifecycle` / power schedule |
| 重启、关机、休眠、唤醒 | `system.lifecycle` |
| 恢复默认、恢复出厂 | `system.reset` or `system.initialization` |

Power 不建议归到 `device`。`device` 描述设备是谁，`system` 描述设备怎么运行。
...
If legacy commands contain schedule fields for shutdown or reboot, map them to system lifecycle or power schedule, not to signage schedule.
...
| `SetDeviceName` | writable `device.info` | Confirm writable display name boundary. |
...
| `GetScheduleConfig` with shutdown/reboot | `system.lifecycle` / power schedule | Do not classify as signage schedule if it controls power. |
...
8. Are power, lifecycle and runtime state owned by system rather than device identity?
