...
| 问题 | 拆层后的处理 |
|---|---|
| USB HID / TCP 需要先协商 MTU、payload type、心跳和预留 ACK 策略 | 放在 CONTROL OPEN / ACCEPT，不污染业务 RPC。 |
...
sequenceDiagram
    ...
    LC->>LS: RPC Identify<br/>op=2, sid="", auth/eventMasks/resumeSid
...
| 消息 | 是否实现 | 为什么需要 | 典型场景 |
|---|---:|---|---|
| `OPEN / ACCEPT` | 是 | 建立 AXTP Framed Link Context，协商版本、MTU、payload type、RPC encoding、心跳间隔和预留 ACK 策略。 | USB HID、TCP、串口网关等需要二进制 Frame 的链路。 |
...
| `controlId` | uint16 LE | `0x0001` | 请求 / 响应匹配序号 |
| `statusCode` | uint16 LE | `0x0000` | OPEN 请求固定填 SUCCESS |
| `body` | TLV | 见下表 | 协商参数 |
...
| 字段 | TLV | 示例值 | 示例编码 | 说明 |
|---|---:|---|---|---|
| `protocolVersion` | `0x02` | `1` | `02 01 01` | AXTP Core 版本 |
...
| `controlId` | uint16 LE | `0x0001` | 必须匹配 OPEN 的 controlId |
| `statusCode` | uint16 LE | `0x0000` | 成功为 0，失败填 ErrorCode |
| `body` | TLV | 见下表 | 最终协商结果 |
...
| 字段 | TLV | 示例值 | 示例编码 | 说明 |
|---|---:|---|---|---|
| `sessionId` | `0x01` | `0x12345678` | `01 04 78 56 34 12` | MVP 可选链路上下文 ID；可保存用于日志/追踪/未来 RESUME，不参与业务路由 |
...
| MessageId | `0x0002` |
| PayloadLength | `35` |
| CRC16-CCITT-FALSE | `0x74c8`，little-endian 写入 `c8 74` |

```text
Header:
41 58 01 01 23 00 02 01 02 00 00 01
...
Full packet:
41 58 01 01 23 00 02 01 02 00 00 01
02 01 00 00 00 01 04 78 56 34 12 02 01 01
04 02 00 10 06 02 c4 09 07 01 07 1e 01 01
0a 02 e8 03 0b 01 00 c8 74
```
...
| Binary 表达 | `uint32` Little-Endian，例如 `78 56 34 12` |
...
Hello 由 Logical Server 发送。它告诉客户端：服务端协议版本、RPC 版本，以及是否需要认证。Phase 1 默认不需要认证；只有启用 OBS-style 时才携带认证挑战参数。
...
| `d.rpcVersion` | uint32 | 是 | RPC 协议版本，当前为 `1` |
...
```text
Header:
41 58 01 02 40 00 02 01 03 00 00 01

Payload:
UTF-8 JSON bytes of the Hello object above
...
Identify 由 Logical Client 发送。它提交客户端期望的 RPC 版本、事件订阅意图，以及可选的认证响应和断线恢复 `resumeSid`。
...
| `op` | uint8 | 是 | `2` |
| `d.rpcVersion` | uint32 | 是 | 客户端支持的 RPC 版本 |
...
```text
Header:
41 58 01 02 3c 00 01 02 04 00 00 01

Payload:
UTF-8 JSON bytes of the Identify object above
...
| `op` | uint8 | 是 | `3` |
| `d.negotiatedRpcVersion` | uint32 | 是 | 协商后的 RPC 版本 |
...
```text
Header:
41 58 01 02 35 00 02 01 05 00 00 01

Payload:
UTF-8 JSON bytes of the Identified object above
...
| Identify 新会话 | `sid=""`；默认不带认证字段，需要密码门禁时才把认证响应放在 `d.authentication`。 |
