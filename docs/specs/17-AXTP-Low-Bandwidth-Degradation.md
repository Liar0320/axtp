# 17《AXTP Low-Bandwidth Degradation》

> Status: AXTP v1 Supplemental Draft
> Spec Version: 0.2.0
> Scope: Compact / HID-64 / BLE / UART degradation guidance

版本：0.2.0
状态：Supplemental Draft
...
| Capability bitmap 派生规则 | `capability.supportedMethods` 继续从 `methods[].bitOffset` 派生 |
...
Compact 草案保留原先的 4B Header + 1B CRC8 设计方向，但它是后续降级 profile 的 wire format，不进入 AXTP v1 Core。
...
CRC8 的具体多项式、初值和 test vector 必须在启用 Compact profile 前进入 registry/source 与 generator snapshot；在此之前不得把 Compact 标记为 stable。
...
在这些事实源进入 registry 之前，`docs/generated/protocol.md` 不应把 Compact 展示为当前可用主线能力。
