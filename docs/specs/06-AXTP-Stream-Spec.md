...
MVP 推荐：AXTP-USB-HID 或 AXTP-TCP 使用 Standard Frame CRC16 + sliding_window 或 stop_and_wait ACK + `verifyType=sha256`。HID-64/BLE/UART 等低带宽降级路径见 17《AXTP Low-Bandwidth Degradation》。
...
低带宽传输不属于 AXTP v1 Core 必选实现。若后续在 HID-64/BLE/UART 上启用 STREAM，必须先证明扣除外层 frame header 和 STREAM Header 后仍有业务数据空间。完整策略见 17《AXTP Low-Bandwidth Degradation》。
