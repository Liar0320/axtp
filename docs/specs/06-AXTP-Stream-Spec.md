...
AXTP STREAM 提供极简、固定的数据面，承载视频帧、音频帧、固件更新数据块、文件块、实时日志、KVM/HID Raw、传感器采样等高吞吐连续数据。
...
**方式 B：领域方法隐式建流**（适用于固件更新、文件、视频等；具体 method 必须来自已采纳业务草案）
...
| `media.video` | `timestampUs` | `best_effort` | draft |
| `media.audio` | `timestampUs`/`sampleIndex` | `best_effort` | draft |
...
- 业务层问题（固件更新 sha256 不匹配、视频帧解码失败、业务处理超时）→ stream.error RPC 事件，触发业务熔断
...
| 视频 | 待视频流草案采纳 | 建流 response | frameIndex | timestampUs |
| 音频 | 待音频流草案采纳 | 建流 response | packetIndex | timestampUs |
...
| seqId 顺序检测 | 必须 |
| cursor 解析 | 必须 |
| 具体业务 profile 的 stop-and-wait | 由已采纳业务/传输 profile 决定 |
| CONTROL ACK/NACK | 必须 |
