...
| `plan-protocol-flow` | 产品/架构师给出业务场景、用户 story、UI 原型或端到端交互时 | 遍历 story 步骤，检索 adopted/generated/draft 协议覆盖，输出 `docs/flows/<scenario>.md` 和缺口清单 | 不写 registry YAML，不把 UI-only 行为塞进协议草案，不运行生成器 |
...
2. 若输入是端到端交互，先运行 `plan-protocol-flow` skill，生成 `docs/flows/<scenario>.md`，列出 story 步骤、协议覆盖和缺口。
