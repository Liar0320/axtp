...
Generator 流程外侧使用一个总控 skill 和五个阶段 skill 辅助治理，各阶段写入边界不同：

| Skill | 何时运行 | 必须检查 | 不允许做什么 |
|---|---|---|---|
| `axtp-protocol-workflow` | 用户没有明确生命周期阶段时 | 判断应进入草案、采纳、生成、维护或 runtime 实现 | 不绕过阶段边界 |
| Stage 10 `plan-protocol-flow` | 产品/架构师给出业务场景、用户 story、UI 原型或端到端交互时 | 遍历 story 步骤，检索 adopted/generated/draft 协议覆盖，输出 `docs/flows/<scenario>.md` 和缺口清单 | 不写 registry YAML，不把 UI-only 行为塞进协议草案，不运行生成器 |
...
| `amend-adopted-protocol` | 已采纳或已生成协议需要语义修正、字段删除、废弃、重命名或扩展时 | 确认目标已 adopted，记录 amendment，判断 draft/experimental vs stable/MVP 兼容策略，修正 proposal/specs/YAML 并重新生成 | 不绕过 adopted proposal/YAML，不静默破坏 stable/MVP，不手写 generated |
...
```text
1. 产品/架构师提交业务流程描述、UI 原型、架构草案或旧协议线索。
2. 若输入是端到端交互，先运行 Stage 10 `plan-protocol-flow` skill，生成 `docs/flows/<scenario>.md`，列出 story 步骤、协议覆盖和缺口。
3. 对缺口或明确的新协议需求运行 `draft-business-protocol` skill，收集 docs/protocol/<domain>/<domain.feature>.md 中所有相关协议方案文件。
4. 读取 docs/specs/08 命名治理、docs/specs/09-13 registry/MVP 表格和 docs/specs/19 generator 规则。
5. 更新协议草案：判断复用现有协议、优化现有草案，或新增 domain.feature 草案；补齐候选 method/event/schema/error/capability/profile、legacyRefs 和待确认问题。
6. 人工评审并确认协议草案；`[REVIEW-ASK]` / `[REVIEW-BLOCKER]` 不得直接进入 YAML。
7. 运行 `adopt-protocol-draft` skill，反向确认 docs/specs/08-13：
...
8. 检查 registry/**/*.yaml 和 registry/domains/**/*.yaml 是否已有等价条目。
9. 默认写入 registry/domains/<domain>/domain.yaml；只有 Core/MVP 晋升或共享基础事实才写入 registry/。
10. 运行 `generate-axtp-protocol` skill，从 YAML 生成 Protocol IR、generated docs、tooling 和 runtime generated 产物。
11. 检查 generated protocol 文档是否完整反映已确认草案。
12. 如果已采纳或已生成协议后续需要语义修正，运行 `amend-adopted-protocol` skill，先更新 adopted proposal/specs/YAML，再重新运行 `generate-axtp-protocol`。
