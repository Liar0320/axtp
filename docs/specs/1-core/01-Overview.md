...
Standard Framed 有 Frame Header，先做 CONTROL OPEN / ACCEPT，再进入 RPC Hello / Identify / Identified，之后运行 RPC 和 STREAM。Phase 1 的 STREAM 数据面重点承载 audio/video 媒体流，固件、文件、日志等 profile 后续增量扩展。

WebSocket Unframed JSON 没有 Frame Header，也没有 CONTROL、STREAM、CRC16、Binary RPC 15B Header。WebSocket 建好后直接发送 JSON RPC Envelope。
...
| PayloadType | 值 | 职责 |
|---|---:|---|
| CONTROL | `0x01` | 协议运行时信令：OPEN、ACCEPT、HEARTBEAT、HEARTBEAT_ACK、CLOSE、CLOSE_ACK；READY/ACK/NACK/RESUME 等保留扩展 |
| RPC | `0x02` | 业务控制面：request、response、event、batch |
| STREAM | `0x03` | 业务数据面：P0 音视频帧，后续固件更新数据块、文件块、日志流 |
...
RPC Payload         rpcEncoding(1B) + encoded sid/op/d envelope
JSON_BINARY Payload rpcEncoding(1B) + 14B fixed envelope + body，仅 rpcEncoding=JSON_BINARY 时使用
STREAM Payload      16B fixed header + data，Standard Framed 使用
```
...
|---|---|---|---|---:|---:|---|
| `AXTP-USB-HID` | Standard Framed | `STANDARD_FRAME` | JSON / CBOR / MSGPACK / JSON_BINARY | 是 | 是 | USB HID 高速/大 report 设备，P0 承载 audio/video STREAM |
| `AXTP-TCP` | Standard Framed | `STANDARD_FRAME` | JSON / CBOR / MSGPACK / JSON_BINARY | 是 | 是 | PC / App 与设备直连，P0 承载 audio/video STREAM |
...
v1 Core 端到端验证链路只固定传输、会话和 RPC 机制；业务链路必须来自已采纳草案：

```text
CONTROL OPEN / ACCEPT
  -> CONTROL HEARTBEAT / HEARTBEAT_ACK, when idle
  -> RPC Hello / Identify / Identified
  -> RPC adopted business request
  -> RPC adopted business event, if declared
  -> STREAM audio/video data after an adopted business method binds streamId
  -> CONTROL CLOSE
```
