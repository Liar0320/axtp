...
RPC Parser 先读取 rpcEncoding。若 rpcEncoding=BINARY，则解析 15B Binary Header：rpcEncoding/rpcOp/sid/requestId/methodOrEventId/statusCode/bodyEncoding，剩余 Payload 按 bodyEncoding 交给 TLV8/TLV16/CBOR/RAW_BYTES Parser；业务层根据 sid、methodOrEventId + schema 解释字段含义。JSON/CBOR/MSGPACK 不使用 Binary Header，也不携带 bodyEncoding/bodyLen。
