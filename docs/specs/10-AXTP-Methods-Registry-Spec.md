...
说明：事件本身使用 EventId，通过 `rpcOp = EVENT` 承载。事件订阅集合由 RPC `IDENTIFY / REIDENTIFY` 的 `eventSubscriptions` 字段声明和更新，MVP 不分配 `event.subscribe / event.unsubscribe` MethodId。本表使用 domain-feature 治理后的主名称，旧名只在 legacy、deprecated 或 migration 说明中出现。
...
| AXDP_HID | `0xB0042` | BetaBrightnessSet | `0x0602` | `display.setBrightness` | 是 | 亮度设置 |
