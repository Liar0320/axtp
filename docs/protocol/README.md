...
|---|---|---:|---|---|
| P1 | `device.*` / `system.*`：[`device.info`](device/device.info.md)、[`device.childDevice`](device/device.childDevice.md)、[`system.state`](system/system.state.md)、[`system.power`](system/system.power.md)、[`system.health`](system/system.health.md)、[`system.lifecycle`](system/system.lifecycle.md)、[`system.reset`](system/system.reset.md)、[`system.initialization`](system/system.initialization.md)、[`system.time`](system/system.time.md) | 64 条，覆盖 AXDP / Rooms / Signage / VM33 | 草案已存在，未进入 generated | 最先采纳生成。它们是设备识别、运行时状态、电源、健康、重启、软重置、初始化、时间同步等通用底座，后续业务和工具都依赖这些基础事实。 |
