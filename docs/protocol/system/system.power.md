...
适用范围：当前设备的供电/电池/电源状态读取、低频电源状态变化通知，以及软件发起的 `powerOff` 下电动作。
...
| `[REVIEW-ASK]` | power schedule | 电源计划/自动下电是否纳入 P0 需要确认。 | 本文先作为可选方法候选。 |
...
| 目标行为 | 查看设备供电、电池、电源模式和 power off 支持情况；执行软件发起的 power off，并监听 power 状态变化。 |
| 当前实现程度 | Not drafted；此前冲突的 `device.power` 草案已迁移并删除。 |
...
| 负责 | power source、battery、charging、power mode、power state、power-off support、software `powerOff`。 |
| 不属于本文 | 主设备身份属于 `device.info`；CPU/内存属于 `system.state`；立即重启、计划重启和 graceful shutdown 属于 `system.lifecycle`；外部 PDU/继电器硬断电不属于 AXTP 本设备软件协议。 |
...
| Schedule | `system.getPowerSchedule` / `system.setPowerSchedule` optional | legacy 有 auto power 线索，但 P0 是否采纳待确认。 |

## 5. 候选 Capability

| Capability | 状态 | 说明 |
|---|---|---|
| `system.power` | draft | 电源状态、供电/电池摘要和软件 power off。 |
...
| `system.getPowerSchedule` | `GetPowerScheduleParams` | `PowerSchedule` | 查询自动开关机/下电计划。 | `[REVIEW-ASK]` |
| `system.setPowerSchedule` | `SetPowerScheduleParams` | `SetPowerScheduleResult` | 设置自动开关机/下电计划。 | `[REVIEW-ASK]` |
...
| `action` | string enum | yes | `power_off` / `power_on`；计划重启使用 `system.scheduleReboot`。 | `[REVIEW-ASK]` |
...
| VM33 / AXDP | `AutoPower` / `AutoShutDown` | `system.getPowerSchedule` / `system.setPowerSchedule` 或 `system.lifecycle` config | `[REVIEW-ASK]` | 需确认是电源计划还是生命周期策略；计划重启使用 `system.scheduleReboot`。 |
...
5. 电源计划是否属于 `system.power`，还是保留在 `system.lifecycle` 的配置策略中？计划重启已归 `system.scheduleReboot`。
