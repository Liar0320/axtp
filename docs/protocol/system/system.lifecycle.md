...
适用范围：系统立即重启、计划重启、graceful shutdown、生命周期状态读取和生命周期状态变化通知。
...
| `[REVIEW-OK]` | reboot/scheduleReboot/shutdown | `system.reboot`、`system.scheduleReboot` 和 `system.shutdown` 是动作型方法，不使用配置型 `setLifecycleConfig` 表达这些动作。 | 进入候选方法。 |
...
| `[REVIEW-OK]` | scheduleReboot 边界 | 计划重启是 lifecycle 动作，归 `system.scheduleReboot`；不放进 `system.power` 电源计划。 | 采纳前确认一次性/周期性计划和取消策略。 |
| `[REVIEW-DRAFT]` | transition event | `system.lifecycleStateChanged` 表达 rebooting、shutting_down、ready 等状态。 | 采纳前确认断连前事件可靠性和重连校准策略。 |
| `[REVIEW-ASK]` | lifecycle config | KeepAlive、auto shutdown 等 legacy 配置是否作为本 feature 的 P1 配置方法仍需确认。 | P0 先聚焦 state/reboot/scheduleReboot/shutdown。 |
...
| 项 | 内容 |
|---|---|
| 需求来源 | `docs/flows/device-system-info.md`、用户要求新增 `reboot` / `scheduleReboot`、legacy reboot/shutdown/keepalive 线索。 |
| 目标用户 | App / PC host / cloud console / device management service。 |
| 目标行为 | 用户触发立即重启、计划重启或 graceful shutdown 后，设备返回接受状态，随后通过生命周期事件或断连/重连让 App 更新 UI。 |
| 当前实现程度 | Drafted only；原草案是泛配置型模板，需要重写为动作和状态协议。 |
...
| 负责 | lifecycle state、immediate reboot、scheduled reboot、shutdown、lifecycle transition event。 |
| 不属于本文 | 软件 power off 属于 `system.power`；CPU/内存属于 `system.state`；健康/温度属于 `system.health`；reset/factory reset 属于 `system.reset` 或 `system.initialization`。 |
...
| 动作 | `system.reboot` / `system.scheduleReboot` / `system.shutdown` | 符合动作型 feature 模板；计划重启不是电源计划。 |
...
| Capability | 状态 | 说明 |
|---|---|---|
| `system.lifecycle` | draft | 系统生命周期状态、立即重启、计划重启和 graceful shutdown。 |
...
| `system.scheduleReboot` | `ScheduleRebootParams` | `ScheduleRebootResult` | 创建或更新一次性/周期性计划重启。 | `[REVIEW-OK]` |
...
|---|---|---|---|
| `system.lifecycleStateChanged` | `LifecycleStateChangedEvent` | ready、reboot_scheduled、rebooting、shutting_down、restarting、error 等生命周期状态变化。 | `[REVIEW-DRAFT]` |
...
| Field | Type | Required | 说明 | Review |
|---|---|---:|---|---|
| `state` | string enum | yes | `ready` / `reboot_scheduled` / `rebooting` / `shutting_down` / `restarting` / `starting` / `error` / `unknown`。 | `[REVIEW-ASK]` |
| `lastAction` | string enum | no | `reboot` / `schedule_reboot` / `shutdown` / `none`。 | `[REVIEW-DRAFT]` |
| `lastActionId` | string | no | 最近一次动作 ID。 | `[REVIEW-DRAFT]` |
| `scheduledReboot` | `ScheduledReboot` | no | 当前计划重启摘要。 | `[REVIEW-DRAFT]` |
...
### `ScheduleRebootResult`

| Field | Type | Required | 说明 | Review |
|---|---|---:|---|---|
| `scheduled` | boolean | yes | 是否已保存计划。 | `[REVIEW-OK]` |
| `schedule` | `ScheduledReboot` | yes | 保存后的计划。 | `[REVIEW-DRAFT]` |
...
| `mode` | string enum | yes | `once` / `daily` / `weekly` / `custom`。 | `[REVIEW-ASK]` |
| `runAt` | string timestamp | no | 一次性重启时间。 | `[REVIEW-DRAFT]` |
| `timeOfDay` | string | no | 周期性重启的本地时间。 | `[REVIEW-DRAFT]` |
...
| `reason` | string enum | no | `user_request` / `scheduled_reboot` / `policy` / `system` / `error`。 | `[REVIEW-ASK]` |
...
### `system.scheduleReboot` response
...
| Error | 类别 | 说明 | Review |
|---|---|---|---|
| `SYSTEM_LIFECYCLE_PERMISSION_DENIED` | system | 无权执行立即重启、计划重启或关机；JSON 示例使用通用 `PERMISSION_DENIED`。 | `[REVIEW-DRAFT]` |
| `SYSTEM_LIFECYCLE_BUSY` | system | 已有 lifecycle/power 动作进行中；JSON 示例使用通用 `BUSY`。 | `[REVIEW-DRAFT]` |
| `SYSTEM_LIFECYCLE_INVALID_STATE` | system | 当前状态不允许 reboot/scheduleReboot/shutdown；JSON 示例使用通用 `INVALID_STATE`。 | `[REVIEW-DRAFT]` |
| `SYSTEM_LIFECYCLE_INVALID_SCHEDULE` | system | 计划重启时间、时区或重复规则非法；JSON 示例可使用 `INVALID_ARGUMENT` / `OUT_OF_RANGE`。 | `[REVIEW-DRAFT]` |
...
| AXDP / VM33 | `RebootInterval` | `system.scheduleReboot` | `[REVIEW-ASK]` | 需要确认旧字段是周期性重启、重启间隔，还是设备本地策略。 |
| AXDP / VM33 | `AutoShutDown` / `AutoPower` | lifecycle config 或 `system.power` schedule | `[REVIEW-ASK]` | 需要确认是 lifecycle 策略还是 power schedule。 |
...
methods:
  ...
  - name: system.reboot
    ...
    capabilities:
      - system.lifecycle
  - name: system.scheduleReboot
    id: TBD after adoption
    bitOffset: TBD after adoption
    domain: system
    requestSchema: ScheduleRebootParams
    responseSchema: ScheduleRebootResult
    capabilities:
      - system.lifecycle
...
- [ ] 10 已确认 state/reboot/scheduleReboot/shutdown 四个方法是否进入 P0。
...
- [ ] legacy RebootInterval 已确定是否进入 `system.scheduleReboot`，KeepAlive/AutoPower/AutoShutdown 已确定是否进入 lifecycle config 或迁移到其他 capability。

## 14. 待确认问题

1. `shutdown` 与 `powerOff` 的状态转换是否共享同一个底层流程？
2. `confirmationToken` 的生成、有效期和权限模型归哪个 domain？
3. `system.scheduleReboot` 是否允许多个计划并存，还是只维护一个默认计划？
4. `system.scheduleReboot` 是否支持取消计划，若支持是否需要 `system.cancelScheduledReboot`？
5. `force` 是否进入 P0，还是只保留 graceful reboot/shutdown？
6. 断连前 event 是否必须发送，还是只作为 best effort？
7. KeepAlive 是否应留在 AXTP session/transport 层，而不是业务 lifecycle 方法？
